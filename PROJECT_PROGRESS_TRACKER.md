...
| 004 | God-object split | P4 | ☐ TODO | | XL — golden first |
...
1. **P1 + P2 core DONE.** 6 tasks complete (006,007,008,018,019,020). Deferred: 005 (blocked), 021/017*/016* (need CI/emulator).
2. **Commit P1+P2** per-task on `dev` (not yet committed). Recommend grouping: (a) P1 reliability, (b) P2 hygiene.
3. **TASK-021 / 017* / 016*:** implement only after a CI dry-run on `windows-2022` + Firebase emulator is available; do not merge blind.
4. **P3 (Security Foundation):** TASK-015 key store + identity keypair is the KEYSTONE — start it next. Blocks TASK-001/002 (E2E crypto, SQLCipher).
5. **TASK-005:** leave BLOCKED until Firebase-emulator proof of missed fast-answer.
6. Do not touch P4 crypto until P3 (TASK-015) merges.
