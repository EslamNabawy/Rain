...
| --- | --- | --- | --- | --- |
| 1 | Push the local account-deletion and scenario-gate changes, then rerun `Build Rain Apps` on the new SHA. | The auth gate and emulator proof now exist locally, but production promotion needs cloud evidence on the exact pushed commit. | [[Scenario Coverage Matrix]], [[Release Gates]], TD-021, BLK-010 | Cloud hard gate records analyze/test/vault/emulator evidence, Android/Windows artifacts, and release-page proof for the new commit. |
| 2 | Finish Firebase terminal room source-of-truth coverage across every runtime path. | Late received frames, terminal cleanup, and late local media signaling writes are mitigated, but full `VoiceCallRuntime` split and device-direction proof are still open. | TASK-003, [[Call State Machine]], [[CallTerminalReconciler]] | Remote terminal room ends local voice calls; late frames/writes stay diagnostic-only; no peer remains active after terminal room state. |
