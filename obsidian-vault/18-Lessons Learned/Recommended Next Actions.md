...
Use [AUTHENTICATION_AUDIT.md](../../AUTHENTICATION_AUDIT.md) and [ROOT_AUTH_STARTUP_REMEDIATION_ROADMAP.md](../../ROOT_AUTH_STARTUP_REMEDIATION_ROADMAP.md) as the current evidence lock for logout/account lifecycle/splash/navigation failures. Phase 1, Phase 2, and Phase 3 have reduced stale cached identity, logout/reset, and split startup-readiness risk; the next app work should move the visual splash/readiness gate high enough that normal app chrome cannot render during unresolved startup phases.

Auth Phase 1 progress 2026-06-03: cached Drift identity is now backend-validated before signed-in restoration. Deleted backend accounts and uid mismatches clear local session, and login/register save local identity only after backend identity/presence writes. Auth Phase 2 progress 2026-06-03: logout clears local session before best-effort backend sign-out, including failed sign-out and logout-after-app-exit shutdown cases. Auth Phase 3 progress 2026-06-03: `AppStartupState` centralizes update/session/runtime/router readiness and tests cover every startup phase. The next auth/startup work is global splash architecture.

## Top Recommended Actions

| Rank | Action | Why Now | Dependencies | Success Criteria |
| --- | --- | --- | --- | --- |
| 1 | Implement the global startup splash/readiness gate. | The startup state machine now exists, but `MaterialApp.router` and shell infrastructure can still be created during post-bootstrap readiness; the visual gate needs to own the whole app surface. | Phase 3 startup state machine, [SPLASH_SCREEN_INVESTIGATION.md](../../SPLASH_SCREEN_INVESTIGATION.md), [NAVIGATION_INITIALIZATION_AUDIT.md](../../NAVIGATION_INITIALIZATION_AUDIT.md) | Normal navigation, runtime widgets, and call overlays cannot render while startup is loading, failed, session-expired, or update-blocked. |
