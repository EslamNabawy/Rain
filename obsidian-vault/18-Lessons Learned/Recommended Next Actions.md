...
Before app fixes resume, use [ROOT_CAUSE_ANALYSIS.md](../../ROOT_CAUSE_ANALYSIS.md) as the current evidence lock for the call/presence/update/diagnostics failure cluster. The next app work should reproduce and fix Firebase end-call permission denial, terminal-state ownership, presence freshness races, and Android diagnostics export before more release builds are trusted.
...
| 2 | Enforce Firebase terminal room state as the call terminal source of truth. | The JSON proves late session failure after terminal room is still becoming a visible last error. | TASK-003, [[Call State Machine]], [[CallTerminalReconciler]] | Remote terminal room ends local voice calls; late frames are diagnostics only, not last-crash records. |
