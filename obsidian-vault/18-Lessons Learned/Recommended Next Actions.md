...
Auth Phase 1 progress 2026-06-03: cached Drift identity is now backend-validated before signed-in restoration. Deleted backend accounts and uid mismatches clear local session, and login/register save local identity only after backend identity/presence writes. The next auth/startup work is deterministic logout/reset and then a dedicated startup readiness state machine.

## Top Recommended Actions

| Rank | Action | Why Now | Dependencies | Success Criteria |
| --- | --- | --- | --- | --- |
| 1 | Implement deterministic logout/reset from [ROOT_AUTH_STARTUP_REMEDIATION_ROADMAP.md](../../ROOT_AUTH_STARTUP_REMEDIATION_ROADMAP.md). | Phase 1 fixed stale cached restoration, but logout still needs guaranteed local clear, runtime disposal, and best-effort backend cleanup ordering. | [[Authentication]], [[Presence Management]], [[Frontend Architecture]] | Logout succeeds locally even if Firebase sign-out/presence cleanup fails; reopening never restores old identity. |
| 2 | Add an explicit auth session coordinator and startup readiness state machine. | Identity validation now exists inside `IdentityController`, but session discovery, runtime readiness, and router readiness still need one orchestration contract. | [AUTHENTICATION_AUDIT.md](../../AUTHENTICATION_AUDIT.md), [STARTUP_SEQUENCE_ANALYSIS.md](../../STARTUP_SEQUENCE_ANALYSIS.md) | App shell renders only after authenticated session and runtime readiness; protected routes cannot render while loading. |
