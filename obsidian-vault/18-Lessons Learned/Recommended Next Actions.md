...
| 4 | Keep isolated Rain app tests on the package-context wrapper. | The full friend-flow suite now passes locally through `scripts/run_rain_app_test.ps1`; future isolated app-test commands should not regress to root-level invocation. | [[Test Strategy]], [[Presence Management]] | New app runtime test instructions and release-gate notes use the wrapper or run from `apps/rain`. |
