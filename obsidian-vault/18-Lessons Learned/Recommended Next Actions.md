...
| --- | --- | --- | --- | --- |
| 1 | Deploy Remote Config and verify old-client update warning. | Fresh `1.0.7+8` artifacts exist in `rain-test-109-1`, but installed apps cannot see the policy until Firebase Remote Config is deployed. | [[Version And Updates]], [[Release Gates]], BLK-004, BLK-006 | A deployed `rain_release_manifest_v1` advertises `1.0.7+8`, old `1.0.6+7` installs show the required update warning, and release evidence records commit/version/channel. |
