...
Governance update 2026-06-04: the root operating manual and Obsidian AI operating notes now encode the Rain Autonomous Engineering System source-of-truth priority order, reality-enforcement rule, and workflow node state machine. The next governance improvement remains [[Engineering System Flaw Remediation Plan]] Phase 02: machine-readable status schemas, followed by a validation evidence ledger.
...
| --- | --- | --- | --- | --- |
| 1 | Implement or explicitly defer account deletion workflow. | Auth source-of-truth, startup, protected navigation, and session lifecycle are now hardened; deleted/reset account behavior remains the remaining auth lifecycle gap. | Phase 6 state lifecycle hardening, [ACCOUNT_LIFECYCLE_ANALYSIS.md](../../ACCOUNT_LIFECYCLE_ANALYSIS.md), TD-021, BLK-010 | User can delete/reset account from the app or the release gate records an explicit accepted deferment with workaround. |
