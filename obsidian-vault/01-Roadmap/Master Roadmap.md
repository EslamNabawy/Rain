...
| Phase 3: Voice Call Runtime Decomposition | P0 | Partial local proof 2026-06-08 | Phases 1-2, [[VoiceCallRuntime Refactor]], [[Call State Machine]] | Call command, media, lock, room reconciliation, terminal reconciliation, peer UI projection, renderer failure authority, error classification, preflight, reconnect, session-state, and signaling-cleanup ownership are split or explicitly bounded. | Characterization tests pass before and after extraction; 2026-06-08 focused state/room/terminal/reconnect/preflight coordinator tests passed locally, and Phase 3c app analyze plus full Melos tests passed with `voice_call_runtime.dart` at 2,917 lines. Full lock/lease/command extraction and device proof remain. |
