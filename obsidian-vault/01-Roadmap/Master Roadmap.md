...
- Progress 2026-06-04/06: The app and release manifests were first bumped to `1.0.7+8` after update warnings did not appear for `1.0.6+7`; `Build Rain Apps` run 26963049075 published `rain-test-109-1`. The 2026-06-06 follow-up found `rain-test-117-1` still used `1.0.7+8`, so `1.0.7+8` installs correctly reported `current`. The app and manifests are now bumped to `1.0.8+9`, and regression coverage proves previous `1.0.6+7` and `1.0.7+8` Android/Windows stable/demo installs receive `updateRequired` from the checked-in Remote Config template. Publishing matching artifacts and live Remote Config deploy/readback remain the operational follow-up.
- Subtasks:
