...
- 2026-06-06 update warning follow-up: current app metadata is being bumped to `1.0.8+9`, and both release manifests advertise `1.0.8+9`. `rain-test-116-1` and `rain-test-117-1` were both `1.0.7+8`, so installed `1.0.7+8` clients correctly evaluated live `1.0.7+8` Remote Config as `current`. `version_metadata_test.dart` now proves the checked-in Remote Config template returns `updateRequired` for previous `1.0.6+7` and `1.0.7+8` stable/demo Android/Windows installs. Live Remote Config must not be deployed to `1.0.8+9` until matching artifacts are published.
