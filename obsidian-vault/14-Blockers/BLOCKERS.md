...
- Progress 2026-06-06: The repeated warning miss after `rain-test-117-1` was another metadata equality case: both the installed app and live policy were `1.0.7+8`. App and manifests are now `1.0.8+9`, and `version_metadata_test.dart` proves previous `1.0.7+8` installs are `updateRequired` from the checked-in template. Closing still requires published `1.0.8+9` artifacts plus live Remote Config deploy/readback and app proof.
