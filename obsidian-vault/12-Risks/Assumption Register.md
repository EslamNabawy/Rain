...
| ASSUMP-008 | Android picker return values are usable filesystem paths. | [[Diagnostics And Logging]], [[File Transfer]] | Picker returns `content://` or `/document/...` handle, or a plugin wrapper tries to reopen that handle with `dart:io`. | Treat as platform-managed handle, write Android SAF exports natively with bytes, and create/report a real fallback file when wrapper failures still occur. | Mitigated |
