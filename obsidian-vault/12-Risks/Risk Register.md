...
| R-003 | Technical | Presence can remain online after app close or stale sessions. | Calls/connects are offered to unavailable peers; offline request notification is blocked. | Medium | High | Use session-owned heartbeat, freshness thresholds, and stale-session rejection. | Presence expiry tests, app-close tests, diagnostics with heartbeat age/session id. | Engineering | [[Presence Management]], [[Presence And Direct Connect]], TASK-006, TD-002 | Open |
