...
| --- | --- | --- | --- | --- | --- | --- |
| SAR-001 | R-001, R-004, R-007, R-009 | Engineering | Critical | Call characterization tests, terminal-state tests, device-direction proof. 2026-06-05 first slice added `CallErrorClassifier` and media-adapter extraction tests; Phase 10 added an opt-in real media capture proof hook, but no Android device proof passed. | Blocks public launch until room reconciliation, lock coordination, command/media ownership, and device-direction proof are complete. | Partial 2026-06-05 |
...
| SAR-003 | R-003, R-016, R-020 | Engineering/Product | Critical | Chat action routing through authoritative peer snapshots. | Local UI action truth is mitigated; Firebase emulator/device proof still blocks release confidence. | Mitigated locally 2026-06-05 |
