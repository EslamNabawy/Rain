...
| SCN-AUTH-004 | Delete account required tombstone failure | Authenticated -> Delete requested -> Reauthenticated -> Tombstone write failed before irreversible backend delete -> Authenticated with visible error | ASSUMP-002 | FG-004, FG-013 | `apps/rain/test/runtime_startup_test.dart` covers provider and direct-runtime pre-tombstone failure preserving session/local identity. `apps/rain/test/settings_screen_test.dart` covers a visible delete-error dialog. `integration_account_deletion_emulator_test.dart` proves the successful emulator tombstone/search cleanup path. `packages/protocol_brain/test/firebase_contract_test.dart` locks the legacy missing-uid rule branch. Live `rain-8fb4b-default-rtdb` rules were deployed/read back with the email-bound missing-uid tombstone path. | Retry the reported Android account against the deployed live rules; add Android smoke evidence if the user-facing Settings delete flow is promoted into the release gate. | BLK-010, BLK-003 | Covered |
| SCN-AUTH-005 | Logout/delete pending teardown exits protected shell and does not stick on loading | Authenticated -> Logout requested -> LocalSessionCleared -> SignedOut while cleanup continues; or Delete requested -> Reauthenticated -> DeletingAccount overlay -> BackendDeleteSucceeded/post-tombstone failure -> RTDBListenersCancelled -> ProtocolSessionsDisconnected -> AuthDeleted -> LocalSessionCleared -> SignedOut; or Authenticated on bad-password/required tombstone failure | ASSUMP-001, ASSUMP-007, ASSUMP-013 | FG-013 | `apps/rain/test/runtime_startup_test.dart` covers logout reaching signed-out while cleanup remains blocked, destructive deletion calling backend delete while runtime cleanup is blocked, bad-password delete restoration, required tombstone failure restoration, Settings remaining mounted during password verification, the full-screen `deletingAccount` phase with navigation hidden, same-device deleted-username marker persistence after successful deletion, account-scoped listener and active protocol session cancellation before Firebase Auth deletion, and connect rejection after shutdown starts. `apps/rain/test/app_routes_test.dart` covers protected shell absence while startup is blocked and route preservation during `deletingAccount`. `apps/rain/test/auth_identity_source_of_truth_test.dart` covers deleted-account login copy when the same device later receives Firebase's generic invalid credential signal. `packages/protocol_brain/test/firebase_contract_test.dart` locks that optional account cleanup is not bundled with the required tombstone in one all-or-nothing RTDB update. Firebase emulator integration passed for account deletion after the cleanup/rule split. | Add device/app smoke evidence if the user-facing Settings teardown flow is promoted into the release gate. | BLK-010 | Covered |
...
| SCN-CALL-006 | Data-peer ICE lifecycle permission safety | Connecting/exchangingIce -> disconnect/recreate -> queued local candidate callback ignored; Connected -> current-session late ICE writes active room | ASSUMP-002, ASSUMP-011 | FG-011 | `protocol_brain_test.dart --plain-name "ICE"` covers live ICE write failure still failing the active session, queued local ICE after disconnect not writing the stale room, and connected sessions keeping the signaling room alive for late local ICE. `rain_debug_log_service_test.dart --plain-name "signaling decorator"` covers sanitized ICE path templates in debug events and `lastCrash.context`. | Add live Firebase/device proof only if repeated diagnostics show active current-room ICE writes still denied after this lifecycle fix. | BLK-003 | Covered |
...
| SCN-MEDIA-010 | Real platform media capture through Rain call media stack | Device/emulator -> request Android media permission when needed -> enumerate media devices -> `DefaultCallMediaConnection.startLocalMedia` -> local audio/video tracks -> dispose | ASSUMP-009 | FG-001 | `apps/rain/integration_test/device_media_reality_proof_test.dart` is an opt-in executable proof using `FlutterWebRTCBridge` and `DefaultCallMediaConnection`. On 2026-06-07, audio-only and audio+video Android emulator proof passed after manual permission grant. The proof now requests/verifies Android microphone/camera permission through Rain's native `rain/media_permissions` channel before WebRTC capture. `scripts/run_device_media_proof.ps1` now launches/installs/pregrants/runs the proof, but latest runner validation lost the emulator/Flutter tool connection before test execution and is not pass evidence. | Add stable Android runner artifact, Windows host proof, and cross-peer call proof. | BLK-001, BLK-007 | Partially Covered |
| SCN-DIAG-001 | Android picker returns platform-managed handle | Exporting diagnostics -> content/document handle -> native platform save or wrapper-failure fallback JSON | ASSUMP-008 | FG-007 | `crash_diagnostics_service_test.dart` covers content URI, `/document/...`, newline-split `/\ndocument/...`, and the `file_picker 12.0.0-beta.3` wrapper failure that throws for `/document/12`. | Add Android smoke evidence if diagnostics export is release-gated. | BLK-005 | Covered |
...
| SCN-DIAG-003 | Sanitizer must not erase the raw event window | Runtime events -> diagnostics export -> recursive sanitizer -> top-level events retained | ASSUMP-008, ASSUMP-011 | FG-012 | `crash_diagnostics_service_test.dart --plain-name "app event log is bounded when exported"` proves the sanitized export keeps the 200 most recent non-coalesced events instead of truncating top-level `events` to 20. | Add a focused regression whenever another top-level diagnostic array needs a non-default sanitizer cap. | BLK-005 | Covered |
| SCN-UPDATE-001 | Remote Config policy is stale or equal after release | LoadingPolicy -> RemotePolicyOutdated/current when policy is not newer | ASSUMP-004 | FG-006 | `force_update_service_test.dart` and settings tests cover stale policy UI. `version_metadata_test.dart` proves the checked-in Remote Config template reports `updateRequired` for previous `1.0.6+7` and `1.0.7+8` stable/demo Android/Windows installs after the `1.0.8+9` metadata bump. `Build Rain Apps` run 27062729519 published `rain-test-118-1`; live Remote Config readback for `rain-8fb4b` version 9 advertises `1.0.8+9`. | Verify old `1.0.7+8` installs show required update and current `1.0.8+9` installs report current against live Remote Config. | BLK-004, BLK-006 | Partially Covered |
...
| P10-MEDIA | Single-device media capture | `apps/rain/integration_test/device_media_reality_proof_test.dart` passes with real microphone and camera tracks. | 2026-06-07 Android emulator proof passed: audio-only mode passed with one audio input and one video input detected; video-required mode initially timed out behind Android runtime permission UI, then passed after `CAMERA` and `RECORD_AUDIO` were granted to `com.rainapp.rain`. The proof now requests/verifies Android permissions through `MainActivity` before WebRTC capture. `scripts/run_device_media_proof.ps1` automates launch/install/pregrant/run; latest runner validation built and installed the APK but the emulator/Flutter tool connection dropped before execution, so no new pass artifact was recorded. | Partially Covered | Run stable Android runner proof, Windows host media proof, and preserve artifacts; keep Android proof in the local release checklist. |
...
- Account deletion tombstone/upsert/search denial now has Firebase emulator proof in `integration_account_deletion_emulator_test.dart`; live `rain-8fb4b-default-rtdb` rules were also deployed/read back after the 2026-06-07 legacy missing-uid ownership hardening.
...
- Local tests cover the main lifecycle, hard release workflows now include account deletion, login/no-recreate, startup protected-route, and bad-password scenarios by ID, and Firebase emulator proof now covers tombstone cleanup plus post-delete denial. The 2026-06-07 provider regressions prove password verification can be pending without moving app startup out of `ready`, keeping Settings mounted for wrong-password feedback, that verified destructive deletion switches to a full-screen `deletingAccount` overlay with navigation hidden while preserving route context for recoverable backend errors, and that account-scoped RTDB listeners plus active protocol sessions are canceled before Firebase Auth deletion. The latest live-rules follow-up proves the deployed `users/{username}` rule contains the missing-uid/email-bound tombstone branch.
