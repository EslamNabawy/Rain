...
- Local tests cover the main lifecycle, hard release workflows now include account deletion, login/no-recreate, startup protected-route, and bad-password scenarios by ID, and Firebase emulator proof now covers tombstone cleanup plus post-delete denial. The 2026-06-07 provider regression also proves password verification can be pending without moving app startup out of `ready`, keeping Settings mounted for wrong-password feedback. The latest live-rules follow-up proves the deployed `users/{username}` rule contains the missing-uid/email-bound tombstone branch.
