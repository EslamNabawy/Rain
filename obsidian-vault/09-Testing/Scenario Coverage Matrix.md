...
| SCN-UPDATE-001 | Remote Config policy is stale or equal after release | LoadingPolicy -> RemotePolicyOutdated/current when policy is not newer | ASSUMP-004 | FG-006 | `force_update_service_test.dart` and settings tests cover stale policy UI. `version_metadata_test.dart` proves the checked-in Remote Config template reports `updateRequired` for previous `1.0.6+7` and `1.0.7+8` stable/demo Android/Windows installs after the `1.0.8+9` metadata bump. `Build Rain Apps` run 26963049075 published earlier `1.0.7+8` artifacts for SHA `f1904e7`; live Remote Config readback for `rain-8fb4b` version 8 advertises `1.0.7+8`. | Publish matching `1.0.8+9` artifacts, deploy/read back live Remote Config to `1.0.8+9`, then verify old `1.0.7+8` installs show required update and current `1.0.8+9` installs report current. | BLK-004, BLK-006 | Partially Covered |
