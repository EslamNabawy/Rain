...
- Branch state observed: synced with `origin/dev` after pushing commits through `f1904e72f1c16773700f0bfa6bcc8ac0fcd7706d`; root `.obsidian/` remains local workspace metadata and is intentionally untracked.
...
- 2026-06-06 update warning follow-up: `rain-test-117-1` reused app metadata `1.0.7+8`, so installed `1.0.7+8` clients correctly saw live `1.0.7+8` policy as current. Metadata and manifests are now `1.0.8+9`, tests prove previous `1.0.6+7` and `1.0.7+8` installs receive required-update policy, `rain-test-118-1` published matching artifacts, and live Remote Config version 9 advertises `1.0.8+9`. Remaining proof is installed old/current app behavior after live fetch.
...
- 2026-06-04 Phase E splash mitigation: Android `LaunchTheme` and `NormalTheme` in both light and night resource folders now use `@drawable/launch_background`, and the launch drawables stay dark `#061017`. Targeted resource contract test `apps/rain/test/android_splash_resources_test.dart` passed locally.
