...
- 2026-06-06 update warning follow-up: `rain-test-117-1` reused app metadata `1.0.7+8`, so installed `1.0.7+8` clients correctly saw live `1.0.7+8` policy as current. Local metadata and manifests are now `1.0.8+9`, with tests proving previous `1.0.6+7` and `1.0.7+8` installs receive required-update policy. Matching artifacts and Remote Config deploy/readback are still required before old installed clients can see the new policy.
