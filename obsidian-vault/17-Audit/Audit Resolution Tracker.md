...
| Phase 5 | Navigation readiness. | [x] Complete | `AppStartupState.canRenderProtectedRoutes` and `usesRoutedAppShell` define protected readiness; unresolved protected paths redirect to `/`; settings/search/friend pages are wrapped in `_ProtectedRouteGate`; signed-out auth uses a standalone Navigator/Overlay and never inserts `RainNavigationShell`; tests cover protected routes while runtime is loading and signed out. | Start state lifecycle hardening. |
